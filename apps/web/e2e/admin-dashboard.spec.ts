...
test('registration queue preserves filters in the URL and keeps review actions safe', async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole('heading', { level: 1, name: /بررسی درخواست دانش‌آموز نمونه یک/ }),
  ).toBeVisible();
