...
export default function AuthLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10 sm:px-6">
      <Image src="/images/hero-city.png" alt="" fill className="object-cover object-center" priority sizes="100vw" />
      ...
      <section className="relative z-10 w-full max-w-[31rem] rounded-[2rem] border border-white/60 bg-white/92 p-6 shadow-[0_30px_90px_-25px_rgba(15,23,42,.65)] backdrop-blur-2xl sm:p-9">
        <div className="flex items-center justify-between gap-4 border-b border-slate-200/70 pb-5">
          <Link href="/" className="inline-flex items-center gap-3 font-black text-foreground">
            <span className="grid size-11 place-items-center rounded-2xl bg-navy text-sun shadow-lg"><BrandMark size={24} /></span>
            <span><span className="block">سامانه سرویس مدرسه</span><span className="mt-0.5 block text-[11px] font-medium text-muted">ورود امن خانواده و مدیریت</span></span>
          </Link>
        ...
        <div className="mt-7 flex items-center justify-center gap-3 border-t border-slate-200/70 pt-5 text-xs text-muted">
          ...
          <Link href="/contact" className="font-bold hover:text-primary">پشتیبانی</Link>
        </div>
      </section>
    </main>
  );
