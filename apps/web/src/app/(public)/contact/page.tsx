...
const channels = [
  { icon: Phone, label: 'تماس تلفنی', value: '۰۲۱-۱۲۳۴۵۶۷۸', desc: 'شنبه تا چهارشنبه ۸:۰۰ تا ۱۸:۰۰', dir: 'ltr' as const },
  { icon: Mail, label: 'ایمیل', value: 'support@schooltransport.ir', desc: 'پاسخگویی ظرف ۲۴ ساعت کاری', dir: 'ltr' as const },
  { icon: MapPin, label: 'نشانی', value: 'تهران، خیابان ولیعصر، بالاتر از میدان ونک، پلاک ۱۲۳۴', desc: 'سامانه سرویس مدرسه — واحد پشتیبانی', dir: 'rtl' as const },
];
      ...
      <section className="relative overflow-hidden surface-dark pb-20 pt-32 sm:pt-36">
        ...
        <PageContainer className="relative z-10">
          <div className="mx-auto max-w-3xl text-center">
            <Badge tone="info" className="mb-4 border-sun/30 bg-sun/15 text-sun backdrop-blur-sm">پشتیبانی</Badge>
            <h1 className="text-balance text-3xl font-black text-white sm:text-4xl">در کنار شما هستیم</h1>
            <p className="mt-4 text-lg text-white/60">تیم پشتیبانی سامانه سرویس مدرسه آماده پاسخگویی به سوالات شماست.</p>
          </div>
        </PageContainer>
      </section>
            ...
            <div className="grid gap-6 sm:grid-cols-3">
              {channels.map((ch, i) => (
                <div key={ch.label} className={cn(
                  ...
                  <p className="mt-1 font-black" dir={ch.dir}>{ch.value}</p>
          ...
          <div className="lg:grid lg:grid-cols-2 lg:items-start lg:gap-12">
            <div>
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  <label htmlFor="contact-name" className="mb-1.5 block text-sm font-bold">نام و نام خانوادگی</label>
                  <Input id="contact-name" placeholder="نام خود را وارد کنید" value={name} onChange={e => setName(e.target.value)} />
                </div>
                <div>
                  <label htmlFor="contact-topic" className="mb-1.5 block text-sm font-bold">موضوع</label>
                  <Select options={topics} value={topic} onValueChange={setTopic} />
                </div>
              </div>
              <div>
                <label htmlFor="contact-message" className="mb-1.5 block text-sm font-bold">پیام شما</label>
                <Textarea id="contact-message" placeholder="پیام خود را بنویسید..." rows={5} value={message} onChange={e => setMessage(e.target.value)} />
              </div>
