...
export function SchoolsDirectory({ schools }: { schools: readonly School[]; source: string }) {
  ...
  const filtered = useMemo(() => {
    return schools.filter((school) => {
      const matchSearch =
        !searchQuery || school.name.includes(searchQuery) || school.city.includes(searchQuery);
          ...
          <div className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-[0_25px_70px_-35px_rgba(15,23,42,.45)] sm:p-7">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
              <div className="relative flex-1">
                <Search
                  aria-hidden="true"
                  className="absolute right-4 top-1/2 size-5 -translate-y-1/2 text-primary"
                />
                <Input
                ...
                <button
                  ...
                  className={cn(
                    ...
                    activeDistrict === d
                      ...
                      : 'border-slate-200 bg-white text-muted hover:border-primary/30 hover:text-primary',
                  )}
                >
                  {d}
                </button>
            ...
            <div className="mt-5 border-t border-slate-100 pt-4 text-sm text-muted">
              <strong className="text-foreground">{filtered.length.toLocaleString('fa-IR')}</strong>{' '}
              مدرسه مطابق جست‌وجوی شما
            </div>
          </div>
        </PageContainer>
      </section>
          ...
          {filtered.length === 0 ? (
            <div className="mx-auto max-w-lg text-center py-12">
              ...
              <p className="mt-1 text-sm text-muted">
                مدرسه‌ای با این شرایط در سامانه ثبت نشده است.
              </p>
            </div>
          ) : (
              ...
              {filtered.map((school, i) => (
                ...
                >
                  ...
                  <div className="flex items-start justify-between">
                    <h2 className="text-lg font-black group-hover:text-primary transition-colors">
                      {school.name}
                    </h2>
                  ...
                  <div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
                    ...
                    <a
                      href="/login"
                      className="inline-flex items-center gap-1 text-xs font-black text-foreground transition group-hover:text-primary"
                    >
                      انتخاب مدرسه <ArrowLeft className="size-3.5" />
                    </a>
                  </div>
                </motion.div>
              ))}
