...
export default async function NotificationsPage() {
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs
        items={[{ label: 'پنل خانواده', href: '/parent/dashboard' }, { label: 'اعلان‌ها' }]}
      />
        ...
        <Card key={item.id}>
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-xs text-muted">{formatJalaliDateTime(item.createdAt)}</p>
            </div>
            <div className="flex items-center gap-2">
              <Badge tone={item.notificationStatus === 'PENDING' ? 'warning' : 'neutral'}>
                {item.notificationStatus === 'PENDING' ? 'خوانده‌نشده' : 'خوانده‌شده'}
              </Badge>
              {item.notificationStatus === 'PENDING' && <MarkReadButton id={item.id} />}
            </div>
          </div>
        </Card>
      ))}
