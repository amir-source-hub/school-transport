...
import { ParentDashboard, type StudentDashboard } from '@/features/parent-dashboard/parent-dashboard';
...
const statusLabel: Record<string, string> = {
  DRAFT: 'پیش‌نویس', SUBMITTED: 'در انتظار بررسی', UNDER_REVIEW: 'در حال بررسی',
  NEEDS_CORRECTION: 'نیازمند اصلاح', APPROVED: 'در انتظار قیمت',
  CONTRACT_PENDING: 'در انتظار قرارداد', CONTRACT_READY: 'قرارداد آماده',
  CONTRACT_ACCEPTED: 'قرارداد پذیرفته‌شده', REJECTED: 'ردشده', CANCELLED: 'لغوشده',
};

export default async function ParentDashboardPage() {
  const [students, enrollments, contracts, payments, notifications] = await Promise.all([
    getStudents(), getEnrollments(), getContracts(), getPayments(), getNotifications(),
  ]);
  const dashboards: StudentDashboard[] = students.map((student) => {
    ...
    return {
      ...
      nextAction: !enrollment ? 'برای این دانش‌آموز درخواست سرویس ثبت کنید.' : status === 'APPROVED' ? 'منتظر اعلام قیمت مدیریت باشید.' : status === 'CONTRACT_READY' ? 'قرارداد را بررسی و تعیین تکلیف کنید.' : 'وضعیت درخواست را در بخش ثبت‌نام دنبال کنید.',
      warning: enrollment?.rejectionReason ?? (status === 'NEEDS_CORRECTION' ? 'اطلاعات درخواست نیازمند اصلاح است.' : null),
      contractStatus: contract?.contractStatus ?? 'هنوز صادر نشده',
      paymentSummary: payment ? `${formatIrr(payment.items.reduce((sum, item) => sum + item.paidAmount, 0))} پرداخت‌شده` : 'برنامه پرداخت هنوز ایجاد نشده',
      nextPayment: nextItem ? formatIrr(nextItem.amount) : payment ? 'تسویه‌شده' : 'پس از پذیرش قرارداد مشخص می‌شود',
      notifications: notifications.slice(0, 3).map((item) => item.message),
    };
  });
