...
export default async function StudentPage({ params }: { params: Promise<{ studentId: string }> }) {
  ...
  const rows = [
    ...
    ['نشانی خانه', student.homeAddress],
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-bold text-primary">نمایه مستقل دانش‌آموز</p>
          <h1 className="mt-1 text-2xl font-black sm:text-3xl">
            {student.firstName} {student.lastName}
          </h1>
        </div>
        <Badge tone={student.status === 'نیازمند اصلاح' ? 'danger' : 'warning'}>
          {student.status}
        </Badge>
      </div>
      <Alert tone="warning" title="فیلدهای حفاظت‌شده">
        کد ملی، مدرسه، پایه، سال تحصیلی و اطلاعات خدمت پس از ارسال مستقیماً قابل تغییر نیستند و
        تغییر آن‌ها به بررسی مدیریت نیاز دارد.
      </Alert>
      <Card>
