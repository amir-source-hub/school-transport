...
export default async function ContractsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      />
      <div>
        <p className="text-sm font-bold text-primary">قراردادهای دانش‌آموزان</p>
        <h1 className="mt-1 text-2xl font-black sm:text-3xl">قراردادها</h1>
      </div>
      {contracts.length === 0 && (
        <Card>
          <p className="text-muted">هنوز قراردادی برای حساب شما صادر نشده است.</p>
        </Card>
      )}
      <div className="grid gap-4 md:grid-cols-2">
        {contracts.map((contract) => (
          <Card key={contract.id}>
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted">
                  {contract.contractNumber} — نسخه {contract.versionNumber}
                </p>
              </div>
              <Badge
                tone={
            ...
            <ButtonLink
              href={`/parent/contracts/${contract.id}`}
              variant="secondary"
