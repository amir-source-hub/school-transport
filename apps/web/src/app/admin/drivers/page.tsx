...
export default async function Page({ searchParams }: { searchParams: Promise<{ q?: string; status?: string }> }) {
  ...
  return <div className="space-y-6"><Breadcrumbs items={[{ label: 'پنل مدیریت', href: '/admin/dashboard' }, { label: 'رانندگان' }]} />
    ...
    <form className="grid gap-3 rounded-2xl border border-border bg-white p-4 sm:grid-cols-[minmax(0,1fr)_12rem_auto]"><Input type="search" name="q" defaultValue={filters.q} placeholder="نام، کد ملی، تلفن یا پلاک…" aria-label="جست‌وجوی رانندگان"/><select name="status" defaultValue={filters.status??'ALL'} className="min-h-12 rounded-xl border border-border bg-white px-3"><option value="ALL">همه وضعیت‌ها</option><option value="ACTIVE">فعال</option><option value="INACTIVE">غیرفعال</option></select><button className="min-h-12 rounded-xl bg-primary px-5 font-bold text-white">اعمال فیلتر</button></form>
