...
export default async function Page({ params }: { params: Promise<{ driverId: string }> }) {
  ...
  const personal: Array<[string, unknown]> = [
    ...
    ['کد ملی', d.nationalId],
    ['جنسیت', driverValueLabel(d.gender)],
    ['همراه', d.phoneNumber],
    ['همراه دوم', d.secondaryPhoneNumber],
    ['تلفن منزل', d.homePhoneNumber],
    ['نام تماس اضطراری', d.emergencyFirstName],
    ['نام خانوادگی تماس اضطراری', d.emergencyLastName],
    ['نسبت تماس اضطراری', d.emergencyRelationship],
    ['تماس اضطراری', d.emergencyPhoneNumber],
    ['تحصیلات', driverValueLabel(d.education)],
    ['شماره شبا', d.iban],
    ['شماره کارت', d.cardNumber],
    ['بانک', d.bankName],
    ['استان', d.province],
    ['شهر', d.city],
    ['منطقه شهرداری', d.municipalityDistrict],
    ['نشانی', d.streetAddress],
    ['کد پستی', d.postalCode],
    ['معرف', d.referrerName],
    ['تلفن معرف', d.referrerPhoneNumber],
  ];
  const vehicle: Array<[string, unknown]> = [
    ['نوع', driverValueLabel(v?.vehicleType)],
    ['سیستم', v?.system],
    ['سال ساخت', v?.modelYear],
    ['ظرفیت', v?.capacity],
    ['نوع استفاده', driverValueLabel(v?.usageType)],
    ['مالکیت', driverValueLabel(v?.ownershipType)],
    ['بیمه', date(v?.insuranceExpiresAt)],
    ['معاینه فنی', date(v?.technicalInspectionExpiresAt)],
  ];
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-muted">پرونده کامل راننده و ارتباط‌های واقعی سرویس</p>
        </div>
        <DriverAdminActions driver={d} vehicle={v} />
      </header>
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <Card>
          ...
          <dl className="mt-4 grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="col-span-2">
              <dt className="mb-2 text-muted">پلاک خودرو</dt>
              <dd>
                <IranianPlate value={String(v?.plateNumber ?? '')} />
              </dd>
            </div>
          </dl>
        </Card>
        <Card className="lg:col-span-2">
          <h2 className="font-black">مسیرها و دانش‌آموزان</h2>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {data.runs.map((run) => (
              <section key={run.id} className="rounded-xl bg-primary-soft p-4">
                <div className="flex justify-between">
                  <h3 className="font-black">{run.title}</h3>
                  <Badge>
                    {run.direction === 'TO_SCHOOL'
                      ? 'رفت'
                      : run.direction === 'FROM_SCHOOL'
                        ? 'برگشت'
                        : 'رفت و برگشت'}
                  </Badge>
                </div>
                ...
                <ul className="mt-3 space-y-1 text-sm">
                  {run.students.map((student) => (
                    <li key={student.id}>
                      ...
                      {student.companion
                        ? ` + همراه (${student.companion.firstName} ${student.companion.lastName})`
                        : ''}
                    </li>
                  ))}
                </ul>
        ...
        <Card className="lg:col-span-2">
          <h2 className="text-lg font-black">تصاویر و مدارک راننده</h2>
          <p className="mt-2 text-sm text-muted">
            هر تصویر را می‌توانید رد کنید؛ سپس فقط همان تصویر در پنل راننده امکان بارگذاری مجدد
            خواهد داشت.
          </p>
        ...
        <Card className="lg:col-span-2 print:border-0">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="font-black">قرارداد راننده</h2>
              <p className="mt-2 text-sm text-muted">
                نسخه {String(d.contractVersion ?? 'driver-v1')} · پذیرفته‌شده در{' '}
                {d.contractAcceptedAt ? formatJalaliDateTime(String(d.contractAcceptedAt)) : '—'}
              </p>
            </div>
            <PrintPageButton />
          </div>
          <div className="mt-4 rounded-xl bg-surface-inset p-5 text-sm leading-8">
            راننده با پذیرش قرارداد، صحت اطلاعات و مدارک، رعایت برنامه سرویس، ایمنی دانش‌آموزان و
            مقررات سامانه را تأیید کرده است.
          </div>
        </Card>
      </div>
    </div>
