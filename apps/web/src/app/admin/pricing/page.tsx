...
export default async function PricingPage() {
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: 'پنل مدیریت', href: '/admin/dashboard' }, { label: 'قیمت‌گذاری' }]} />
      <div>
          ...
          return (
            <Card key={record.id}>
              ...
              <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-3">
                <div><dt className="text-muted">قیمت فعلی</dt><dd className="mt-1 font-bold">{record.price === null ? 'ثبت نشده' : formatIrr(record.price)}</dd></div>
                <div><dt className="text-muted">وضعیت قرارداد</dt><dd className="mt-1 font-bold">{record.contractStatus}</dd></div>
