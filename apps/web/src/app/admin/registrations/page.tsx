...
export default async function RegistrationsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const filtered = demoRegistrations
    ...
    .filter(
      (item) =>
        !query ||
        `${item.trackingCode} ${item.student} ${item.family} ${item.school}`.includes(query),
    )
    .toSorted((a, b) =>
      sort === 'student'
        ? a.student.localeCompare(b.student, 'fa')
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-2 text-sm text-muted">
          فیلتر، مرتب‌سازی و صفحه در نشانی صفحه حفظ می‌شوند؛ داده‌ها نمایشی‌اند.
        </p>
      </div>
      <Card>
        <form method="get" className="grid gap-4 md:grid-cols-[1fr_13rem_13rem_auto] md:items-end">
          ...
          <label className="text-sm font-bold">
            ...
            <select
              name="status"
              defaultValue={status}
              className="mt-2 min-h-12 w-full rounded-[var(--radius-sm)] border border-border bg-surface px-3 text-sm"
            >
              ...
              <Card key={item.id}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted">{item.trackingCode}</p>
                  </div>
                ...
                <dl className="mt-4 grid gap-2 text-sm">
                  <div>
                    ...
                    <dd className="font-bold">{item.family}</dd>
                  </div>
                  <div>
                ...
                <tbody>
                  {visible.map((item) => (
                    <tr key={item.id} className="border-b border-border last:border-0">
                      ...
                      <td className="px-3 py-3">{item.family}</td>
