...
export function ParentDashboard({ students }: { students: readonly DemoStudentDashboard[] }) {
  ...
  return (
    <div className="space-y-6">
      <Breadcrumbs items={[{ label: "پنل خانواده", href: "/parent/dashboard" }, { label: "نمای کلی" }]} />

      <section className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-bold text-primary">نمای کلی خانواده</p>
          <h1 className="mt-1 text-2xl font-black sm:text-3xl">وضعیت هر دانش‌آموز را جداگانه پیگیری کنید</h1>
          <p className="mt-2 text-sm text-muted">تمام اطلاعات این صفحه نمایشی است و از سرور دریافت نشده است.</p>
        </div>
        <ButtonLink href="/parent/students/new">افزودن دانش‌آموز</ButtonLink>
      </section>

      <section aria-labelledby="student-selector-title">
        <h2 id="student-selector-title" className="text-sm font-black">دانش‌آموز فعلی</h2>
        <div className="mt-3 flex gap-3 overflow-x-auto pb-2" role="group" aria-label="انتخاب دانش‌آموز">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4" aria-label="خلاصه وضعیت دانش‌آموز">
        <Card>
          <GraduationCap aria-hidden="true" className="size-6 text-primary" />
          <p className="mt-4 text-sm text-muted">ثبت‌نام</p>
          <div className="mt-2"><Badge tone={selectedStudent.enrollmentTone}>{selectedStudent.enrollmentStatus}</Badge></div>
      ...
      <section className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
        <Card>
          ...
          <dl className="mt-4 divide-y divide-border text-sm">
            <div className="flex flex-col gap-1 py-3 sm:flex-row sm:justify-between"><dt className="text-muted">نام</dt><dd className="font-bold">{selectedStudent.name}</dd></div>
            <div className="flex flex-col gap-1 py-3 sm:flex-row sm:justify-between"><dt className="text-muted">مدرسه و پایه</dt><dd className="font-bold">{selectedStudent.schoolAndGrade}</dd></div>
            <div className="flex flex-col gap-1 py-3 sm:flex-row sm:justify-between"><dt className="text-muted">سال تحصیلی</dt><dd className="font-bold">{selectedStudent.academicYear}</dd></div>
          </dl>
        </Card>
        <Card>
          <div className="flex items-center gap-2"><Bell aria-hidden="true" className="size-5 text-primary" /><h2 className="text-lg font-black">اعلان‌های اخیر</h2></div>
          <ul className="mt-4 space-y-3 text-sm">
            {selectedStudent.notifications.map((notification) => (
              <li key={notification} className="rounded-[var(--radius-sm)] bg-surface-muted p-3">{notification}</li>
            ))}
          </ul>
        </Card>
      </section>
