  ...
  return (
    <form
      ...
      onSubmit={async (event) => {
        ...
        setError(undefined);
        try {
          await configureOfflineDestination({
            ...form,
    ...
    >
      ...
      <label className="flex items-start gap-3 rounded-xl border border-warning/30 bg-warning/5 p-4 text-sm">
        <input
          type="checkbox"
          checked={confirmed}
          onChange={(e) => setConfirmed(e.target.checked)}
        />
        <span>اطلاعات مقصد را بررسی کرده‌ام و تأیید می‌کنم که جایگزین نسخه فعال شود.</span>
      </label>
      {error && (
      ...
      <Button type="submit" loading={pending} disabled={pending || !confirmed}>
        ذخیره نسخه جدید مقصد پرداخت
      </Button>
    </form>
  );
}
