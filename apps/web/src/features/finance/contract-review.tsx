...
export function ContractReview({
  ...
  useEffect(() => {
    queueMicrotask(() => {
      try {
        ...
        if (stored.page >= 1 && stored.page <= 3) setPage(stored.page);
        setReviewed(stored.reviewed?.filter((item) => [1, 2, 3].includes(item)) ?? [1]);
  ...
  function move(nextPage: number) {
    if (nextPage < 1 || nextPage > 3 || nextPage > Math.max(...reviewed) + 1) return;
  ...
  function download() {
    const body = pages
      .map(
        (items, index) =>
          `<section class="page"><h1>صفحه ${index + 1} از 3</h1>${items.map((item) => `<p>${escapeHtml(item)}</p>`).join('')}</section>`,
      )
      .join('');
  ...
  return (
    <section aria-labelledby="contract-viewer-title" className="space-y-4">
      ...
      <p aria-live="polite" className="text-center text-sm font-black print:hidden">
        صفحه {page.toLocaleString('fa-IR')} از ۳
      </p>
      <article
        ref={articleRef}
        tabIndex={0}
      ...
      <div className="flex justify-between gap-3 print:hidden">
        <Button variant="ghost" disabled={page === 1} onClick={() => move(page - 1)}>
          صفحه قبل
        </Button>
        <Button disabled={page === 3} onClick={() => move(page + 1)}>
          صفحه بعد
        </Button>
      </div>
