      ...
      <section className="rounded-[var(--radius-lg)] border border-border bg-surface p-5">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          ...
          <Field label="شماره همراه پدر">
            <Input value={demoFamilyProfile.fatherMobile} dir="ltr" disabled aria-describedby="phone-help" />
          </Field>
          <Field label="شماره همراه مادر">
            <Input value={demoFamilyProfile.motherMobile} dir="ltr" disabled aria-describedby="phone-help" />
          </Field>
          <div id="phone-help" className="md:col-span-2 text-sm text-muted">
            تغییر شماره اصلی نیازمند فرایند جداگانه تأیید با رمز یک‌بارمصرف است. شماره اصلی فعلی: <span dir="ltr" className="font-bold text-foreground">{demoFamilyProfile.primaryPhone}</span>
          </div>
          <Field label="ایمیل" error={errors.email?.message}>
            <Input {...register("email")} type="email" dir="ltr" aria-invalid={Boolean(errors.email)} />
          </Field>
        </div>
      </section>
      ...
      <section className="rounded-[var(--radius-lg)] border border-border bg-surface p-5">
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          ...
          <Field label="نام کامل تماس اضطراری" error={errors.emergencyContactName?.message}>
            <Input {...register("emergencyContactName")} aria-invalid={Boolean(errors.emergencyContactName)} />
          </Field>
          <Field label="نسبت با دانش‌آموز" error={errors.emergencyContactRelationship?.message}>
            <Input {...register("emergencyContactRelationship")} aria-invalid={Boolean(errors.emergencyContactRelationship)} />
          </Field>
          <Field label="شماره همراه تماس اضطراری" error={errors.emergencyContactMobile?.message}>
            <Input {...register("emergencyContactMobile")} inputMode="numeric" dir="ltr" aria-invalid={Boolean(errors.emergencyContactMobile)} />
          </Field>
        </div>
      </section>

      {saved && <Alert title="تغییرات نمایشی ذخیره شد">این پیام تأیید فقط از آداپتور نمایشی دریافت شده است.</Alert>}
