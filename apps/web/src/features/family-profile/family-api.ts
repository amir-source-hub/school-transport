...
const parentSchema = z.object({
  id: z.string(), parentType: z.string(), firstName: z.string(), lastName: z.string(),
  nationalId: z.string(), phoneNumber: z.string(), isPrimaryContact: z.boolean(), phoneVerified: z.boolean(),
});
const addressSchema = z.object({
  id: z.string(), title: z.string(), province: z.string(), city: z.string(),
  district: z.string().optional(), streetAddress: z.string(), postalCode: z.string().optional(), isActive: z.boolean(),
});
const emergencySchema = z.object({
  id: z.string(), firstName: z.string(), lastName: z.string(), relationship: z.string(), phoneNumber: z.string(), isActive: z.boolean(),
});
export const familyProfileSchema = z.object({
  id: z.string(), username: z.string(), mother: parentSchema.nullable(), father: parentSchema.nullable(),
  addresses: z.array(addressSchema), emergencyContacts: z.array(emergencySchema),
});
...
export async function updateParent(parentType: 'MOTHER' | 'FATHER', firstName: string, lastName: string) {
  await apiRequest('/families/me', { method: 'PATCH', body: { parentType, firstName, lastName } });
}
