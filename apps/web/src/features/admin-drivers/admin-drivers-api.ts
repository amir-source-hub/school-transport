...
export type DriverDetail = {
  ...
  runs: Array<{ id: string; title: string; direction: string; academicYear: string; scheduledStartTime: string; scheduledArrivalTime: string; school: { id: string; name: string }; students: Array<{ id: string; firstName: string; lastName: string; pickupOrder: number; seatCount:number; companion:null|{id:string;firstName:string;lastName:string;relationship:string} }> }>;
  documents: Array<{ id: string; documentType: string; mimeType: string; viewUrl: string; reviewStatus: 'ACTIVE' | 'REJECTED'; rejectionReason: string | null }>;
};
export type AdminTransportRoute = {
  id: string; driverId: string; title: string; direction: 'TO_SCHOOL' | 'FROM_SCHOOL' | 'ROUND_TRIP'; academicYear: string;
  scheduledStartTime: string; scheduledArrivalTime: string; activeWeekdays: number[]; areaDescription: string | null;
...
export async function rejectAdminDriverDocument(driverId:string,documentId:string,reason?:string){return (await apiRequest(`/admin/drivers/${driverId}/documents/${documentId}/reject`,{method:'POST',body:{reason}})).data;}
export async function updateAdminDriver(id:string,body:Record<string,string>){return (await apiRequest(`/admin/drivers/${id}`,{method:'PATCH',body})).data;}
export async function deactivateAdminDriver(id:string){return (await apiRequest(`/admin/drivers/${id}`,{method:'DELETE'})).data;}
...
export async function createAdminTransportRoute(body: { driverId:string;schoolId:string;title:string;academicYear:string;direction:'TO_SCHOOL'|'FROM_SCHOOL'|'ROUND_TRIP';scheduledStartTime:string;scheduledArrivalTime:string;areaDescription?:string;activeWeekdays:number[] }) { return (await apiRequest('/admin/transport-routes', { method: 'POST', body })).data; }
export async function updateAdminTransportRoute(routeId:string,body:{driverId?:string;schoolId?:string;title?:string;academicYear?:string;direction?:'TO_SCHOOL'|'FROM_SCHOOL'|'ROUND_TRIP';activeWeekdays?:number[]}) { return (await apiRequest(`/admin/transport-routes/${routeId}`, { method:'PATCH', body })).data; }
