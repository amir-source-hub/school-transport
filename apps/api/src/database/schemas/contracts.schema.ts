...
export const contracts = pgTable(
  'contracts',
  {
    ...
    acceptedByAdminId: uuid('accepted_by_admin_id').references(() => adminUsers.id),
    signerRole: varchar('signer_role', { length: 30 }).notNull().default('PARENT'),
    signerReason: varchar('signer_reason', { length: 500 }),
    signerSource: varchar('signer_source', { length: 30 }),
  ...
  (table) => ({
    ...
    regVersionIdx: uniqueIndex('idx_contracts_reg_version').on(
