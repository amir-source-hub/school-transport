import { pgTable, uuid, varchar, integer, timestamp, boolean, index, uniqueIndex } from 'drizzle-orm/pg-core';
import { serviceRegistrations } from './registrations.schema';
import { adminUsers } from './auth.schema';

export const registrationPrices = pgTable('registration_prices', {
  ...
  setByAdminId: uuid('set_by_admin_id').notNull().references(() => adminUsers.id),
