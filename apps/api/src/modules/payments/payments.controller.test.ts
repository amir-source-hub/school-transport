...
import { AdminPaymentsController, OnboardingPaymentsController, PaymentsController } from './payments.controller';
...
describe('payment controller authorization contracts', () => {
  it('protects payer and onboarding routes with their respective guards', () => {
    ...
    expect(Reflect.getMetadata(GUARDS_METADATA, OnboardingPaymentsController)).toContain(OnboardingGuard);
  });

  it('limits destination configuration and receipt review to active admins', () => {
    expect(Reflect.getMetadata(GUARDS_METADATA, AdminPaymentsController)).toEqual([AuthGuard, RolesGuard]);
    expect(Reflect.getMetadata(ROLES_KEY, AdminPaymentsController)).toEqual(['ADMIN']);
    for (const method of ['configureOfflineDestination', 'offlineSubmissions', 'approve', 'reject'] as const) {
