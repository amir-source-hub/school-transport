...
describe('receipt image processor', () => {
  it.each(['jpeg', 'png'] as const)('normalizes a valid %s to bounded sRGB JPEG without metadata', async (format) => {
    let pipeline = sharp({ create: { width: 1200, height: 1800, channels: 3, background: '#eeeeee' } });
    pipeline = format === 'png' ? pipeline.png() : pipeline.jpeg();
    const result = await processReceiptImage(await pipeline.withMetadata({ orientation: 6 }).toBuffer(), limits);
  ...
  it('rejects huge dimensions before decoding the image', async () => {
    const image = await sharp({ create: { width: 9000, height: 2, channels: 3, background: '#fff' } }).png().toBuffer();
