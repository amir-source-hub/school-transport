...
export async function processReceiptImage(input: Buffer, limits: { maxBytes: number; maxPixels: number; maxAxis: number }) {
  if (input.length === 0 || input.length > limits.maxBytes) throw new ReceiptImageError('INVALID_SIZE');
  ...
  if (!metadata.width || !metadata.height || metadata.width > limits.maxAxis || metadata.height > limits.maxAxis) {
    throw new ReceiptImageError('INVALID_DIMENSIONS');
  }
  const canonical = await image.rotate().resize({ width: 1600, height: 1600, fit: 'inside', withoutEnlargement: true }).flatten({ background: '#ffffff' }).jpeg({ quality: 88, mozjpeg: true }).toColorspace('srgb').toBuffer();
