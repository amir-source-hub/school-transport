...
export const STUDENT_EXPORT_COLUMNS = [
  ...
  ['nationalId', 'کد ملی دانش آموز'],
  ['phoneNumber', 'تلفن دانش آموز'],
  ['birthDate', 'تاریخ تولد دانش آموز به تقویم ایرانی'],
  ['school', 'مدرسه'],
  ['educationLevel', 'مقطع تحصیلی'],
  ['grade', 'پایه'],
  ['guardianRelationship', 'نسبت سرپرست'],
  ['guardianName', 'نام و نام خانوادگی سرپرست'],
  ['guardianNationalId', 'کد ملی سرپرست'],
  ['guardianPhone', 'تلفن سرپرست'],
  ['fatherName', 'نام و نام خانوادگی پدر'],
  ['fatherNationalId', 'کدملی پدر'],
  ['fatherPhone', 'تلفن پدر'],
  ['motherName', 'نام و نام خانوادگی مادر'],
  ['motherNationalId', 'کد ملی مادر'],
  ['motherPhone', 'تلفن مادر'],
  ['emergencyName', 'نام و نام خانوادگی اضطراری'],
  ['emergencyRelationship', 'نسبت اضطراری'],
  ['emergencyPhone', 'تلفن اضطراری'],
  ['homePhone', 'تلفن منزل (ثابت)'],
  ['province', 'استان'],
  ['city', 'شهر'],
  ['district', 'منطقه'],
  ['address', 'نشانی'],
  ...
  ['hasCompanion', 'مراقب دانش آموز'],
  ['companionName', 'نام و نام خانوادگی مراقب'],
  ['companionNationalId', 'کد ملی مراقب'],
  ['companionPhone', 'تلفن مراقب'],
  ['serviceType', 'متقاضی سرویس'],
...
export function formatIranianExportDate(
...
const relationship = (value?: string | null, description?: string | null) =>
  value === 'FATHER'
    ? 'پدر'
    : value === 'MOTHER'
      ? 'مادر'
...
const serviceLabel = (value?: string | null) =>
  ({ VAN: 'ون', CAR: 'سواری', BUS: 'اتوبوس', MINIBUS: 'مینی بوس' })[
    value as 'VAN' | 'CAR' | 'BUS' | 'MINIBUS'
  ] ?? '';

export async function createStudentWorkbook(
  database: DatabaseService,
  maxRows: number,
): Promise<Buffer> {
  const limit = maxRows + 1;
  const studentRows = await database.db
    .select()
    .from(students)
    .orderBy(asc(students.id))
    .limit(limit);
  if (studentRows.length > maxRows)
    throw new ValidationError('تعداد دانش‌آموزان برای خروجی هم‌زمان بیش از حد مجاز است.');
  const parentRows = await database.db.select().from(parents).orderBy(asc(parents.id)).limit(limit);
  const addressRows = await database.db
    .select()
    .from(familyAddresses)
    .orderBy(asc(familyAddresses.id))
    .limit(limit);
  const emergencyRows = await database.db
    .select()
    .from(emergencyContacts)
    .orderBy(asc(emergencyContacts.id))
    .limit(limit);
  const companionRows = await database.db
    .select()
    .from(studentCompanions)
    .orderBy(asc(studentCompanions.id))
    .limit(limit);
  const schoolRows = await database.db.select().from(schools).orderBy(asc(schools.id)).limit(limit);
  const registrationRows = await database.db
    .select()
  ...
  const planRows = await database.db
    ...
    .limit(limit);
  const scheduleRows = await database.db
    .select()
  ...
  if (
    [
      ...
      planRows,
      scheduleRows,
    ].some((rows) => rows.length > maxRows)
  )
    throw new ValidationError('داده‌های گزارش برای خروجی هم‌زمان بیش از حد بزرگ است.');

  const familyParents = new Map<string, typeof parentRows>();
  for (const parent of parentRows)
    familyParents.set(parent.userId, [...(familyParents.get(parent.userId) ?? []), parent]);
  const activeAddress = new Map(
    addressRows.filter((row) => row.isActive).map((row) => [row.userId, row]),
  );
  const addressById = new Map(addressRows.map((row) => [row.id, row]));
  const activeEmergency = new Map(
    emergencyRows.filter((row) => row.isActive).map((row) => [row.userId, row]),
  );
  ...
  for (const price of priceRows)
    pricesByRegistration.set(price.registrationId, [
      ...(pricesByRegistration.get(price.registrationId) ?? []),
      price,
    ]);
  ...
  for (const item of scheduleRows)
    itemsByPlan.set(item.paymentPlanId, [...(itemsByPlan.get(item.paymentPlanId) ?? []), item]);
  ...
  for (const registration of registrationRows) {
    ...
    if (!previous || registration.createdAt > previous.createdAt)
      latestRegistration.set(registration.studentId, registration);
  }

  const workbook = new ExcelJS.Workbook();
  workbook.creator = 'سامانه سرویس مدارس';
  const sheet = workbook.addWorksheet('دانش‌آموزان', {
    views: [{ state: 'frozen', xSplit: 3, ySplit: 1, rightToLeft: true, showGridLines: false }],
    properties: { defaultRowHeight: 24 },
  });
  sheet.columns = STUDENT_EXPORT_COLUMNS.map(([key, header]) => ({
    key,
    header,
    width: Math.min(38, Math.max(18, header.length + 4)),
  }));
  for (const student of studentRows) {
    ...
    const familyHomePhone = relatives.find((row) => row.homePhone?.trim())?.homePhone ?? '';
    const service = latestRegistration.get(student.id);
    const selectedAddress = service?.selectedAddressId
      ? addressById.get(service.selectedAddressId)
      : undefined;
    const address =
      selectedAddress?.userId === student.userId
        ? selectedAddress
        : activeAddress.get(student.userId);
    ...
    const educationLevel =
      school?.educationOptions?.find((option) => option.grades.includes(student.grade ?? ''))
        ?.level ??
      student.className ??
      '';
    const prices = service ? (pricesByRegistration.get(service.id) ?? []) : [];
    const latestPrice = prices
      .filter((price) => planByPrice.has(price.id))
      .sort(
        (a, b) =>
          Number(b.priceStatus === 'ACCEPTED') - Number(a.priceStatus === 'ACCEPTED') ||
    ...
    const paymentStatus = !plan
      ? 'برنامه پرداخت ثبت نشده'
      : plan.planStatus === 'CANCELLED'
        ? 'لغو شده'
        : plan.planStatus === 'COMPLETED' || (items.length > 0 && paidItems === items.length)
          ? 'پرداخت کامل'
          : paidItems > 0
            ? 'پرداخت جزئی'
            : 'پرداخت نشده';
    sheet.addRow({
      ...
      birthDate: formatIranianExportDate(student.birthDate),
      school: school?.name ?? '',
      educationLevel,
      grade: student.grade ?? '',
      guardianRelationship: relationship(
        guardian?.relationshipType ?? guardian?.parentType,
        guardian?.relationshipDescription,
      ),
      ...
      homePhone: guardian?.homePhone || father?.homePhone || mother?.homePhone || familyHomePhone,
      ...
      hasCompanion: companion ? 'دارد' : 'ندارد',
      companionName: name(companion),
      companionNationalId: companion?.nationalId ?? '',
      companionPhone: companion?.phoneNumber ?? '',
      serviceType: serviceLabel(service?.serviceType),
      physicalStatus:
        student.physicalStatus === 'SPECIAL'
          ? 'استثنائی'
          : student.physicalStatus === 'HEALTHY'
            ? 'سالم'
            : '',
      disabilityType: student.disabilityType ?? '',
      createdAt: formatIranianExportDate(
        userById.get(student.userId)?.createdAt ?? student.createdAt,
        true,
      ),
  ...
  sheet.autoFilter = {
    from: { row: 1, column: 1 },
    to: { row: Math.max(1, sheet.rowCount), column: STUDENT_EXPORT_COLUMNS.length },
  };
