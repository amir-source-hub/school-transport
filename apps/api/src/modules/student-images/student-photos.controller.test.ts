...
import { AdminStudentPhotosController, OnboardingStudentPhotosController, StudentPhotosController } from './student-photos.controller';
...
describe('student photo controller authorization', () => {
  it('separates account, onboarding, and ordinary-admin boundaries', () => {
    ...
    expect(Reflect.getMetadata(GUARDS_METADATA, OnboardingStudentPhotosController)).toContain(OnboardingGuard);
    expect(Reflect.getMetadata(GUARDS_METADATA, AdminStudentPhotosController)).toEqual([AuthGuard, RolesGuard]);
