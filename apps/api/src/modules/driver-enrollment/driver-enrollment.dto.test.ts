...
import { CreateTransportRouteDto, DriverEnrollmentDto, UpdateDriverProfileDto } from './driver-enrollment.dto';

describe('DriverEnrollmentDto', () => {
  it('requires a 24-digit IR IBAN, 16-digit card number, and bank name', async () => {
    const valid = plainToInstance(DriverEnrollmentDto, { iban:'IR123456789012345678901234', cardNumber:'6037991234567890', bankName:'بانک ملی' });
    const invalid = plainToInstance(DriverEnrollmentDto, { iban:'IR123', cardNumber:'123', bankName:'' });
    expect((await validate(valid)).map(error=>error.property)).not.toEqual(expect.arrayContaining(['iban','cardNumber','bankName']));
    expect((await validate(invalid)).map(error=>error.property)).toEqual(expect.arrayContaining(['iban','cardNumber','bankName']));
  });
...
describe('CreateTransportRouteDto contract terms', () => {
  const route = { driverId:'11111111-1111-4111-8111-111111111111', schoolId:'22222222-2222-4222-8222-222222222222', title:'مسیر نمونه', academicYear:'1405-1406', direction:'ROUND_TRIP', scheduledStartTime:'07:00', scheduledArrivalTime:'17:00', activeWeekdays:[0,1], contractPriceRials:2000000, contractDate:'1405/06/22' };
  ...
  it('rejects negative prices and invalid Persian dates', async () => {
    const errors = await validate(plainToInstance(CreateTransportRouteDto, { ...route, contractPriceRials:-1, contractDate:'2026-09-13' }));
    expect(errors.map(error=>error.property)).toEqual(expect.arrayContaining(['contractPriceRials','contractDate']));
  });
});
