...
- Student-photo decoding and canonicalization now execute in an actual child process with a 128 MB JavaScript heap ceiling, two-process concurrency cap, bounded IPC output, libvips timeout, parent wall-clock kill, and fail-closed invalid-output handling. Crafted JPEG/PNG polyglots with trailing executable/archive data and four concurrent highly compressed over-pixel inputs are regression-tested while parent heap growth remains bounded. Byte, pixel, axis, aspect-ratio, decode-failure, renamed SVG/PDF, corrupt, empty, and oversized cases remain covered.
...
- The real-service CI gate runs every migration against PostgreSQL, then proves notification/feedback isolation and stable pagination, photo-approval uniqueness, payment ownership/duplicate idempotency, row locks, exact-once offline finalization, durable Redis client recovery, and killed-worker process recovery. Deployment CI builds the production API/web images, starts the real Compose dependency chain, waits for service health, and performs HTTP health requests from inside both running containers. Compatibility of migration `0025` with a production-like legacy snapshot remains solely the staging-specific blocker already listed above.
- The real Redis CI gate now queues an outbox-shaped job, starts a separate BullMQ worker process that claims and holds it, kills that process, starts a replacement worker, and proves BullMQ stalled-lock recovery completes the same durable job. This complements the queue-client restart proof and unit-level stale-processing/retry/dead-letter transitions.
...
- The API generates a deterministic committed OpenAPI snapshot from the real Nest module graph, and the web now generates committed TypeScript path, parameter, request-body, and response types from that canonical snapshot with `openapi-typescript`. Frontend CI is triggered by both web changes and `apps/api/openapi.json`, regenerates the types, and fails on drift; runtime web feature boundaries continue to parse untrusted responses with Zod. Building the real module graph also exposed and fixed invalid S3 provider construction and a missing queue-to-photo module dependency.
...
- A versioned machine-checked privacy register classifies all 31 database tables exactly once by purpose, subjects, data classes, access, retention state, and deletion behavior, and records least-privilege, bounded/audited export, encrypted backup/restore, and evidence controls. Security CI fails when a schema table is missing or stale. Runtime ownership/role checks, private short-lived object access, raw-object cleanup, student/archive and session-revocation controls, minimized audits, bounded exports, encrypted backups, and guarded restore validation enforce the locally decidable controls.
- [ ] Approve exact retention, erasure/anonymization, dispute/legal-hold, consent-history, child-record, photo, receipt, financial, audit, support, and campaign periods; only then enable irreversible cleanup jobs and update the register from `pending-legal-approval`. **[BLOCKED — LEGAL/PRODUCT POLICY]**
- A 28-day SLO/error-budget policy now defines API availability/latency, PostgreSQL readiness, durable queue, OTP-provider, and critical-browser objectives with platform, security, finance, and messaging ownership. Masked structured logs carry request IDs and validated W3C trace IDs; protected low-cardinality Prometheus metrics cover normalized HTTP status/latency, PostgreSQL pool/readiness, queue enqueue/completion/failure, provider outcomes/latency, notification backlog, and broadcast spend. Importable Grafana panels, platform/messaging alert rules, and a privacy-safe incident runbook cover triage, containment, worker recovery, guarded database restore, payment ambiguity, disclosure escalation, and closure. Security CI verifies that every critical signal, owner, and response section remains represented.
...
- A CI-enforced quality matrix now requires all ten layers: API/web unit tests, interactive component tests, real PostgreSQL/Redis integration, critical browser E2E, axe accessibility, 320 px/768 px mobile coverage, platform-independent full-page visual baselines, deterministic bundle plus scheduled real-browser performance budgets, dependency/storage failure and retry flows, and backend/frontend regression workflows. The approved public landing baseline is generated and re-compared in Chromium at desktop, phone, and tablet sizes; missing evidence files fail CI rather than silently narrowing coverage.
- Enrollment Playwright coverage now runs at 320 px and 768 px with the real long form: Jalali date and constrained image inputs are exposed, a workflow choice survives reload, the validation summary appears, focus moves to the first invalid field, the actual map/manual-address step renders and accepts keyboard focus, and neither step creates document overflow. Draft recovery is deliberately limited to a two-hour tab session and an explicit allowlist of non-identifying workflow choices (gender, coarse city/province, school/grade, service/payment selection); tests prove names, national IDs, phones, street/coordinates, notes, and photo IDs are never persisted, and completed submissions clear the draft. Existing component coverage retains sticky/navigation, geolocation denial/retry, field validation, and upload behavior.
...
- A machine-enforced abuse matrix maps every required category to executable evidence and fails security CI if files or named assertions disappear. Coverage includes refresh reuse/session revocation and concurrent OTP consumption/brute-force expiry; exact Origin/Referer CSRF controls; real-PostgreSQL notification/feedback/payment IDOR plus ownership/role guards; literal notification HTML/script rendering in real browsers with no DOM execution plus CSP; spreadsheet/formula and malformed DTO injection; a fixed-origin map proxy that rejects invalid grids before upstream access; isolated photo polyglots/bombs and hostile receipt uploads; OTP cooldown and broadcast recipient/spend caps; secret/phone/OTP/log redaction; and generic safe error responses that suppress raw messages/stacks.
