...
- [ ] Finish the repository-wide list audit and move genuinely large exports to queued/streamed generation. Existing paged operational queues enforce validated maximum page sizes and stable ordering; the broadcast campaign view is limited to the newest 200 campaigns with stable timestamp/ID ordering and count queries scoped to that window; comprehensive previews are capped at 50 rows per page, while the synchronous XLSX export reads every source in stable ID order with a 10,000-row-per-source ceiling and fails closed before workbook construction when any source exceeds it. **[NOT FINISHED]**
