...
- [ ] Add the remaining flow-specific PostgreSQL proofs for row locks, concurrent payment idempotency/authorization, notification migration, and offline-payment finalization, plus a deployed-process smoke after those database cases pass. Notification/feedback isolation and pagination plus the photo-approval uniqueness race now run in the real-service CI gate. **[NOT FINISHED]**
