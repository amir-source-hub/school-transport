...
- [ ] Build a reviewed asset inventory and migration manifest before deleting anything. The current candidates are the runtime WebP files under `apps/web/public/images`; classify each path by page/component, owner, rights/license, dimensions, byte size, MIME type, checksum, accessibility text, target object key, and replacement URL. Keep `apps/web/src/app/favicon.ico`, required manifest/icon assets, small code-owned SVGs, and PNG visual-regression baselines in Git unless a separate reviewed reason proves they are runtime content. **[READY — STATIC ASSET INVENTORY]**
- [ ] Define immutable object naming and cache semantics. Prefer content-hashed or versioned keys such as `public/site/<release-or-hash>/<descriptive-name>.webp`, correct `Content-Type`, long-lived `Cache-Control: public, max-age=31536000, immutable`, strong ETag/checksum verification, and a stable HTTPS CDN/custom asset domain. Never overwrite an immutable key; upload a new key and update references so rollback remains possible. **[READY — ASSET DELIVERY DESIGN]**
...
- [ ] Add a documented build-time public asset base such as `NEXT_PUBLIC_ASSET_BASE_URL` and a single URL helper/manifest rather than scattering bucket URLs through components. Validate an absolute HTTPS production origin, allow loopback/local fallback only in development, configure Next.js remote-image policy/loader behavior narrowly for the chosen hostname, and ensure the asset origin contains no credentials or signed query parameters. **[READY — WEB ASSET CONFIGURATION]**
- [ ] Replace every `/images/...` runtime reference in public pages, authentication/admin backgrounds, carousel data, registration-guide data, CSS backgrounds, metadata/social cards, preload hints, and tests with the manifest/helper output. Preserve `next/image` width/height or `fill`/`sizes`, responsive crops, loading priority, Persian alt text, layout stability, and graceful error/fallback behavior; do not migrate inline decorative SVG generated by React. **[READY — WEB REFERENCE MIGRATION]**
- [ ] Create an idempotent, non-secret asset publishing procedure that reads the manifest, uploads with exact MIME/cache metadata, compares checksums, refuses unexpected overwrite/deletion, supports dry-run, reports missing/orphaned objects, and can run locally or in release CI with a write-only scoped credential. Do not commit provider credentials, generated presigned URLs, bucket listings, or deployment logs containing sensitive configuration. **[READY — ASSET PUBLISHING TOOLING]**
...
- [ ] Remove migrated runtime image files from the current Git tree and Git LFS only after uploaded checksums, deployed references, monitoring, and a rollback release are verified. Update `.gitattributes`, `.dockerignore`, Docker build context, LFS checkout requirements, CI assertions, tests, documentation, and license/source records without removing LFS rules still needed by screenshot baselines or other retained binary fixtures. **[READY — REPOSITORY CLEANUP AFTER CUTOVER]**
...
- [ ] Keep the previous application release and static objects available through a measured rollback window. Rollback must restore the exact prior code-to-object manifest without re-uploading mutable filenames; delete unreferenced objects only after access logs, retention policy, legal/license requirements, backups, and rollback expiry confirm they are no longer needed. **[READY — ASSET ROLLBACK/LIFECYCLE]**
